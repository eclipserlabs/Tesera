...
def canonicalize(
    ...
    replaced with :data:`REDACTED` if it matches. Additionally, any string
    value matching one of *patterns* (e.g. ``sk-live-...`` under a generic
    name like ``data``) is replaced with :data:`REDACTED`. The replacement is
    a fixed literal, so no length, prefix, or hash of the raw value survives.

    Raises:
        CanonicalizationError: for NaN/infinity, cycles, or excessive depth.
    """
    collected: list[str] = []
    result = _walk(value, sensitive, patterns, collected, _depth=0, _seen=frozenset())
...
def _walk(
    ...
    *,
    ...
    if isinstance(value, str):
        if value and any(p.search(value) is not None for p in patterns):
            return _redact(value, collected)
        return value
    ...
    def descend(item: Any, seen: frozenset[int]) -> Any:
        return _walk(item, sensitive, patterns, collected, _depth=_depth + 1, _seen=seen)
...
def canonical_hash(
    ...
    return sha256_hex(canonical_json_bytes(canonicalize(value, sensitive, patterns).value))
